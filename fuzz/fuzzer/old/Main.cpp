...
void MakeDirectory(void) {
    struct stat buffer;
    if (!sync_id.empty() && stat(sync_dir.c_str(), &buffer) != 0) { // use multiple fuzzer, and sync_dir does not exist
...
int main(int argc, char** argv) {    
    ...
    SetupSignalHandlers(); cout << "SetupSignalHandlers() success" << endl;
    ShmSetup(); cout << "ShmSetup() success" << endl;
    init_count_class(); cout << "init_count_class() success" << endl;
    ReadTestcases(inputDir); cout << "ReadTestcases() success" << endl;
    ...
    if (sync_cov == 1 && !sync_id.empty()) {
        ...
        if (access(file_lock.c_str(), 0) != 0) { // file_lock does not exist
    ...
    DryRunning(testArg); cout << "DryRunning() success" << endl;
    CullQueue(); cout << "CullQueue() success" << endl;
...
int main(int argc, char** argv) {
    ...
    if (WIFSIGNALED(status)) {
		...
		if (killSignal == SIGKILL) return FAULT_TMOUT;
		cout << "\nFAULT_CRASH" << endl;
        return 0;
	}
