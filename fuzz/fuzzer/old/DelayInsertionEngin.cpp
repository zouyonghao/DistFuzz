                                 ...
                                 matrix_element *tmp_travel_matrix)
{
    ...
    {
        if (tmp_travel_matrix[i].hash_1 == 0 &&
            tmp_control_matrix[i].hash_2 == 0)
            break;

        for (int j = 0; j < TRAVEL_MATRIX_SIZE; j++)
        {
            if (tmp_travel_matrix[j].hash_1 == 0 &&
                tmp_travel_matrix[j].hash_2 == 0)
                break;

            if (compare_matrix_element(tmp_control_matrix[i],
                                       tmp_travel_matrix[j]))
            {
                return true;
            }
...
u8 one_element_cm_test(char **argv, u8 *filemap, u64 len, u64 hash_1,
                       u64 hash_2)
{
    ...
    cout << "travel_matrix: "
         << num_of_matrix_elemet(travel_matrix, TRAVEL_MATRIX_SIZE) << endl;
    cout << "total_travel: " << total_travel.size() << endl;
    cout << "unreachable_control_matrix: " << unreachable_control_matrix.size()
         << endl;
    cout << "-----------------------------------" << endl;
    return 0;
}

u8 mutate_based_on_adjacency(char **argv, u8 *filemap, u64 len,
                             matrix_element *matrix, int matrix_len,
                             matrix_element *tmp_am)
{
    for (int i = 0; i < matrix_len; i++)
    {
        ...
        for (int j = 0; j < ADJACENCY_MATRIX_SIZE; j++)
        {
            ...
            if (cm_hash1 == am_hash1 &&
                one_element_cm_test(argv, filemap, len, cm_hash2, am_hash2))
                return 1;
            if (cm_hash1 == am_hash2 &&
                one_element_cm_test(argv, filemap, len, cm_hash2, am_hash1))
                return 1;
            if (cm_hash2 == am_hash1 &&
                one_element_cm_test(argv, filemap, len, cm_hash1, am_hash2))
                return 1;
            if (cm_hash2 == am_hash2 &&
                one_element_cm_test(argv, filemap, len, cm_hash1, am_hash1))
                return 1;
        }
    }
    return 0;
                         ...
                         matrix_element *tmp_control_matrix)
{
    if (FuzzUnit(argv, filemap, len, NULL, tmp_control_matrix))
        return 1;
    print_result("ConcurrencyFuzzing");
    memcpy(tmp_adjacency_matrix, adjacency_matrix,
           ADJACENCY_MATRIX_SIZE * sizeof(matrix_element));
    memcpy(tmp_travel_matrix, travel_matrix,
           TRAVEL_MATRIX_SIZE * sizeof(matrix_element));

    memset(enable_control_matrix, 0,
    ...
    cout << "START: unreachable_control_matrix: "
         << unreachable_control_matrix.size() << endl;

    // if has enable control matrix
    if (enable_num > 0)
    {
        if (mutate_based_on_adjacency(argv, filemap, len, enable_control_matrix,
                                      enable_num, tmp_adjacency_matrix))
            return 1;
    }

    // if dont has enable control matrix, just choose from tmp_travel_matrix
    if (mutate_based_on_adjacency(argv, filemap, len, tmp_travel_matrix,
                                  TRAVEL_MATRIX_SIZE, tmp_adjacency_matrix))
        return 1;

    cout << "END : concurrency_test_time: " << concurrency_test_time << endl;
    cout << "END : total_travel: " << total_travel.size() << endl;
    cout << "END : unreachable_control_matrix: "
         << unreachable_control_matrix.size() << endl;
