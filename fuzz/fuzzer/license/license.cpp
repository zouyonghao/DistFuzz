...
static string g_encrypt_str = "default_encrypt_str";
...
string RSA_DecryptFile(string file)
{
    ...
    if (RSA_public_decrypt(len, (unsigned char *)(cipper_data), (unsigned char *)text_data, rsa, RSA_PKCS1_PADDING) < 0)
    {
...
bool CheckLicenseFile(string &checker_str)
{
    ...
    char tmp_str[100];
    sscanf(begin_time_str.c_str(), "%s %d-%d-%d", tmp_str, &begin_year, &begin_month, &begin_day);
    sscanf(end_time_str.c_str(), "%s %d-%d-%d", tmp_str, &end_year, &end_month, &end_day);
...
bool CheckWriteRunLogFile()
{
    ...
    sscanf(str.c_str(), "%s %d-%d-%d-%d:%d:%d: %s", tmp_str, &my_time1->year, &my_time1->month, &my_time1->day,
           &my_time1->hour, &my_time1->minute, &my_time1->second, tmp_str);
    ...
    char tmp[200];
    sprintf(tmp, "[EVENT] %d-%d-%d-%d:%d:%d: RUN\n", my_time2->year, my_time2->month, my_time2->day, my_time2->hour,
            my_time2->minute, my_time2->second);
    delete my_time2;
