...
echo st > "$PIPE_FOLDER"calc$GET_A_SERVER_ID.pipe
sleep 1
LOG_ID=$(expr $GET_A_SERVER_ID - 1)
if [[ $1 == 'set' ]]; then
    echo "should set"
    echo "LOG_ID is $LOG_ID"
    LEADER_ID=$(tail -n 5 "$LOG_FOLDER"log"$LOG_ID"_* | grep leader | head -n 1 | awk '{print $3}')
    if [ -z "$LEADER_ID" ]; then
    ...
    echo "should set $2"
    echo !$2 >"$PIPE_FOLDER"calc$LEADER_ID.pipe
    sleep 1

    LOG_ID=$(expr $LEADER_ID - 1)
    if tail -n 2 "$LOG_FOLDER"log"$LOG_ID"_* | grep succeed; then
        echo "set success"
        echo $2
    else
        echo "set failed"
    ...
    echo "should get"
    VALUE=$(tail -n 2 "$LOG_FOLDER"log"$LOG_ID"_* | head -n 1 | awk '{print $4}')
    echo $VALUE
fi
