...
public:
    ...
    bool _do() override
    {
        ...
        case ACTION_TYPE::READ:
        {
            ...
            if (aerospike_key_select(&as, &err, NULL, &key, bins, &p_rec) !=
                    AEROSPIKE_OK ||
                p_rec == NULL)
            {
                fprintf(stderr, "aerospike_key_get() returned %d - %s\n",
                        err.code, err.message);
                __dst_event_record(
                    get_result_record(op_name, op_vector, -1, "FAIL").c_str());
                return false;
            }
            char *val_as_str =
                as_val_tostring(as_bin_get_value(p_rec->bins.entries));
            printf("read value is %s\n", val_as_str);

            __dst_event_record(
                get_result_record(op_name, op_vector, 0, val_as_str).c_str());
            free(val_as_str);
        ...
        case ACTION_TYPE::WRITE:
        {
            ...
            if (aerospike_key_put(&as, &err, &wpol, &key, &rec) != AEROSPIKE_OK)
            {
                ...
                if (err.code != AEROSPIKE_ERR_TIMEOUT)
                {
                    printf("write failed!\n");
                    __dst_event_record(
                        get_result_record(op_name, op_vector, -1, "FAIL")
                            .c_str());
                }
                else
                {
            ...
            __dst_event_record(
                get_result_record(op_name, op_vector, 0, "").c_str());
...
REGISTER_NORMAL_OPERATOR(AerospikeWrite,
                         new AerospikeClient(ACTION_TYPE::WRITE));
